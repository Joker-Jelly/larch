...
### Added
- **System Service Management (`larch service`)**: New CLI command to register larch as a system service. Supports `install`, `uninstall`, and `status` subcommands for both launchd (macOS) and systemd (Linux).
- **Internal Install Script**: Rewritten `install.sh` to fetch releases from `code.byted.org` instead of GitHub, with interactive prompt to install as a system service after binary installation.

### Changed
- Removed Windows support and GitHub API calls from `install.sh`.
- Architecture mapping in `install.sh` now uses `x86_64`/`arm64` directly (no `amd64` mapping).
